...
const ModelSelector: React.FC<ModelSelectorProps> = ({ dropdownDirection = 'down' }) => {
  ...
  const handleModelSelect = (model: typeof availableModels[0]) => {
    ...
    // Persist selected model to config so main process (cowork runner) can read it
    configService.updateConfig({
      model: { defaultModel: model.id, defaultModelProvider: model.providerKey || model.provider } as any,
    });
    setIsOpen(false);
  };

  // If no available models, show a hint
