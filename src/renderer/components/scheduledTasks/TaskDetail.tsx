...
const dtLocale = () => i18nService.getDateLocale();

function formatScheduleLabel(schedule: Schedule): string {
  switch (schedule.type) {
    case 'at':
      return `${i18nService.t('scheduledTasksScheduleAtLabel')}: ${schedule.datetime ? new Date(schedule.datetime).toLocaleString(dtLocale()) : '-'}`;
    case 'interval': {
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className={valueClass}>
              {task.expiresAt
                ? new Date(task.expiresAt + 'T00:00:00').toLocaleDateString(dtLocale())
            ...
            <div className={valueClass}>
              ...
              {task.state.lastRunAtMs && (
                <span className="ml-1 text-xs dark:text-claude-darkTextSecondary text-claude-textSecondary">
                  ({new Date(task.state.lastRunAtMs).toLocaleString(dtLocale())})
                </span>
              )}
            </div>
          </div>
          <div>
            ...
            <div className={valueClass}>
              {task.state.nextRunAtMs
                ? new Date(task.state.nextRunAtMs).toLocaleString(dtLocale())
                : '-'}
            </div>
          </div>
