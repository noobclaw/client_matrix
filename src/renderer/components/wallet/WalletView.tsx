...
export const WalletView: React.FC<WalletViewProps> = ({ isSidebarCollapsed, onToggleSidebar, onNewChat, updateBadge, onShowInvite }) => {
  ...
  //   选 BNB 时按实时汇率把美元换算成 BNB 数再下单(见 handlePayPackageWith)。
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-5 max-w-3xl mx-auto w-full space-y-4">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* 账密账号:绑定收款钱包(profile 由 /api/user/profile 带回
                  account_type / payout_wallet 两个新字段) */}
              {(profile as any)?.account_type === 'password' && (
                <PayoutWalletRow
        ...
        <div>
          ...
          {step === 'select' && (
            <>
              ...
              /* ─── 档位卡片(统一美元)───
                 支付方式不在这里选 —— 点卡片后弹窗选(银行卡 / USDT / BNB / 官方店铺卡密)。 */
              <div className="grid grid-cols-3 gap-3">
        ...
        {/* ── 选择支付方式弹窗(点档位卡片后弹)──
          顺序即推荐序:银行卡(推荐)→ USDT → BNB → 官方店铺(卡密)。
