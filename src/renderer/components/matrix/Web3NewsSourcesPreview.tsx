 ...
 *
 * 拉后端 GET /api/web3/news-sources(分源 + 每源最近 5 条),按源卡片展示,点条目开原文。
 * 选材池口径与运行时 AI 取材(newsPicker)一致 → 用户能预览 AI 会从哪些源、近期有哪些热点取材。
 * 仿「全网热搜」(GlobalHotSearchPage)的源卡片做法,精简到能塞进向导一步里。
 */
  ...
  return (
    <div>
      <div className="text-[11px] text-gray-500 dark:text-gray-400 mb-2">
        {isZh
          ? `📰 AI 会从这 ${sources.length} 个 web3 资讯源近 3 周的热点里取材(各源最近 5 条,点标题看原文):`
          : `AI picks from these ${sources.length} web3 sources, last 3 weeks (latest 5 each, click to open):`}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {sources.map((src) => (
          <div key={src.source} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/40 overflow-hidden">
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-gray-100 dark:border-gray-800">
              <span className="text-sm">{emojiOf(src.source)}</span>
