...
const CoworkQuestionWizard: React.FC<CoworkQuestionWizardProps> = ({
  ...
  const handleSelectOption = (question: QuestionItem, optionLabel: string) => {
    ...
    if (!question.multiSelect) {
      // Single-select mode: set answer directly
      setAnswers((prev) => {
        const newAnswers = {
          ...prev,
          [question.question]: optionLabel,
        };
        console.log('[CoworkQuestionWizard] single-select - new answers:', newAnswers);
        return newAnswers;
      });

      // Auto-advance to the next question after single-select (delayed to show selection effect)
      setTimeout(() => {
        // Use functional update to get the latest currentStep
      ...
      setAnswers((prev) => {
        ...
        console.log('[CoworkQuestionWizard] multi-select - current value:', rawValue);

        // If rawValue is empty, directly add the new option
        if (!rawValue.trim()) {
          const newAnswers = {
            ...prev,
            [question.question]: optionLabel,
          };
          console.log('[CoworkQuestionWizard] multi-select - first selection:', newAnswers);
          return newAnswers;
        }

        // Otherwise parse existing values and toggle
        ...
        console.log('[CoworkQuestionWizard] multi-select - parsed set:', Array.from(current));

        if (current.has(optionLabel)) {
          current.delete(optionLabel);
          console.log('[CoworkQuestionWizard] multi-select - deselected:', optionLabel);
        } else {
          current.add(optionLabel);
          console.log('[CoworkQuestionWizard] multi-select - selected:', optionLabel);
        }

        // If empty after deletion, return empty string
        if (current.size === 0) {
          const newAnswers = { ...prev };
          delete newAnswers[question.question];
          console.log('[CoworkQuestionWizard] multi-select - all options deselected:', newAnswers);
          return newAnswers;
        }
        ...
        console.log('[CoworkQuestionWizard] multi-select - updated answers:', newAnswers);
        return newAnswers;
      });
    }
  };
