...
function mxTaskToScenario(t: any): ScenarioTaskIPC {
  ...
  return {
    ...
    scenario_id: isReply ? `${t.platform}_reply_fans_comment` : isDownload ? `${t.platform}_video_download` : isImageText ? `${t.platform}_image_text` : isViral ? `${t.platform}_viral_production_career` : isRepost ? 'binance_repost' : (isTweet || isBinancePost || isFacebookPost) ? `${t.platform}_post` : engageScenarioIdForPlatform(t.platform),
    track: isReply ? 'reply_fan_comment' : isDownload ? 'video_download' : isImageText ? 'image_text' : isViral ? 'viral_production' : isTweet ? 'x_post' : isBinancePost ? 'binance_post' : isFacebookPost ? 'facebook_post' : isRepost ? 'binance_repost' : 'matrix',
    // image_text / viral_rewrite / x_post / binance_post / facebook_post / binance_repost 配置透传(详情页/编辑回填 + updateTask 兜底不丢配置)。
