...
const IDENTITY_EXPR: Record<string, string> = {
  ...
  // 推特X:左侧账号切换器(SideNav_AccountSwitcher_Button)里有 昵称 + @handle + 头像;handle 也可从
  //   UserAvatar-Container-<handle> 的 testid 拿(更稳);uid 从 twid=u%3D<id> cookie。【未真机验,DOM 改版需调】
  x: '(function(){try{var btn=document.querySelector(\'[data-testid="SideNav_AccountSwitcher_Button"]\');var nickname=null,handle=null,avatar=null;if(btn){var img=btn.querySelector("img");avatar=img&&img.src||null;var sp=btn.querySelectorAll("span");for(var i=0;i<sp.length;i++){var t=(sp[i].textContent||"").trim();if(/^@[A-Za-z0-9_]{1,20}$/.test(t)){handle=t;}else if(t&&!nickname&&t.length<40&&t.indexOf("@")!==0){nickname=t;}}}if(!handle){var av=document.querySelector(\'[data-testid^="UserAvatar-Container-"]\');if(av){var m=(av.getAttribute("data-testid")||"").match(/UserAvatar-Container-(.+)/);if(m)handle="@"+m[1];}}if(!avatar){var im2=document.querySelector(\'[data-testid^="UserAvatar-Container-"] img\');avatar=im2&&im2.src||null;}var uid=null;var c=(document.cookie||"").match(/twid=u(?:%3D|=)(\\d+)/);if(c)uid=c[1];return JSON.stringify({nickname:nickname,displayId:handle,uid:uid,avatar:avatar});}catch(e){return "{}";}})()',
