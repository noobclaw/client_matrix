...
export async function downloadUpdate(
  ...
  const ts = Date.now();
  const downloadPath = path.join(tempDir, `noobclaw-update-${ts}${ext}.download`);
  const finalPath = path.join(tempDir, `noobclaw-update-${ts}${ext}`);
...
async function installWindowsNsis(exePath: string): Promise<void> {
  ...
  // the NSIS customInit macro runs `taskkill /IM "NoobClaw.exe" /F /T`
  ...
  const ts = Date.now();
  const tempDir = app.getPath('temp');
  const logPath = path.join(tempDir, `noobclaw-update-${ts}.log`);
  const scriptPath = path.join(tempDir, `noobclaw-update-${ts}.ps1`);
  const vbsPath = path.join(tempDir, `noobclaw-update-${ts}.vbs`);
