...
function getLabels(store: SqliteStore): { showWindow: string; newTask: string; settings: string; quit: string } {
  try {
    ...
    return lang === 'en'
      ? { showWindow: 'Open NoobClaw', newTask: 'New Task', settings: 'Settings', quit: 'Quit' }
      : { showWindow: '打开 NoobClaw', newTask: '新建任务', settings: '设置', quit: '退出' };
  } catch {
    return { showWindow: '打开 NoobClaw', newTask: '新建任务', settings: '设置', quit: '退出' };
  }
}
