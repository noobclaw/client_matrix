...
if command -v node > /dev/null 2>&1; then
  NODE_BIN="node"
elif [ -n "${NOOBCLAW_ELECTRON_PATH:-}" ] && [ -x "${NOOBCLAW_ELECTRON_PATH}" ]; then
  NODE_BIN="$NOOBCLAW_ELECTRON_PATH"
  NODE_ENV_PREFIX=("ELECTRON_RUN_AS_NODE=1")
else
  echo '{"success":false,"error":"未找到 Node.js 运行时。请安装 Node.js 或确保 NoobClaw Electron 可用。"}' >&2
  exit 1
fi
