...
resolve_node_runtime() {
  ...
  if [ -n "${NOOBCLAW_ELECTRON_PATH:-}" ] && [ -x "${NOOBCLAW_ELECTRON_PATH}" ]; then
    NODE_CMD="$NOOBCLAW_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from NoobClaw."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from NoobClaw so scripts can use Electron runtime."
  exit 1
fi
