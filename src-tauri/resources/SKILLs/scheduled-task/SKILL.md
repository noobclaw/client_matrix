...
| `notifyPlatforms` | ❌ | 任务完成后自动发送结果的 IM 平台：`["dingtalk","feishu","wecom","qq","telegram","discord"]`（默认 `[]`）。用户说"发到钉钉/飞书/企业微信/QQ/TG"时设置此字段，**不要**写进 prompt |
